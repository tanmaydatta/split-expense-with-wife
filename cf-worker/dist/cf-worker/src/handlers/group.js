...
export async function handleGroupDetails(request, env) {
    ...
    return withAuth(request, env, async (session, db) => {
        ...
        const groupUsers = usersResult.map((user) => ({
            Id: user.id,
        ...
        const response = {
            ...
            budgets: JSON.parse(group.budgets || "[]"),
            users: groupUsers,
...
export async function handleUpdateGroupMetadata(request, env) {
    ...
    return withAuth(request, env, async (session, db) => {
        ...
        if (body.defaultShare !== undefined || body.defaultCurrency !== undefined) {
            const currentGroup = await db
                ...
                .where(eq(groups.groupid, session.group.groupid))
                .limit(1);
            const currentMetadata = JSON.parse(currentGroup[0]?.metadata || "{}");
            const newMetadata = {
                ...currentMetadata,
        ...
        const updatedGroup = await db
            ...
            .where(eq(groups.groupid, session.group.groupid))
            .limit(1);
        const updatedMetadata = JSON.parse(updatedGroup[0]?.metadata || "{}");
        const response = {
