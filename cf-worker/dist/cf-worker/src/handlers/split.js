import { createJsonResponse, createErrorResponse, withAuth, formatSQLiteTime, calculateSplitAmounts, validateSplitPercentages, validatePaidAmounts, isValidCurrency, generateDrizzleBalanceUpdates, generateRandomId, } from "../utils";
...
export async function handleSplitNew(request, env) {
    ...
    try {
        return withAuth(request, env, async (session, db) => {
            ...
            const splitAmounts = calculateSplitAmounts(body.amount, body.paidByShares, body.splitPctShares, body.currency);
            ...
            const userInserts = splitAmounts.map((split) => db.insert(transactionUsers).values({
                transactionId: transactionId,
...
export async function handleSplitDelete(request, env) {
    if (request.method !== "POST") {
        return createErrorResponse("Method not allowed", 405, request, env);
    }
    try {
        return withAuth(request, env, async (session, db) => {
            ...
            const transactionDetails = await db
                ...
                .where(and(eq(transactionUsers.transactionId, body.id), eq(transactionUsers.groupId, session.group.groupid), isNull(transactionUsers.deleted)));
            ...
            const deleteTransaction = db
                ...
                .where(and(eq(transactions.transactionId, body.id), eq(transactions.groupId, session.group.groupid)));
            const deleteTransactionUsers = db
                .update(transactionUsers)
                .set({ deleted: deletedTime })
                .where(and(eq(transactionUsers.transactionId, body.id), eq(transactionUsers.groupId, session.group.groupid)));
...
export async function handleTransactionsList(request, env) {
    if (request.method !== "POST") {
        return createErrorResponse("Method not allowed", 405, request, env);
    }
    try {
        return withAuth(request, env, async (session, db) => {
            ...
            const rawTransactionsList = await db
                .select()
                .from(transactions)
                .where(and(eq(transactions.groupId, session.group.groupid), isNull(transactions.deleted)))
            ...
            if (transactionIds.length > 0) {
                ...
                const allDetails = await db
                    ...
                    .where(and(inArray(transactionUsers.transactionId, transactionIds), eq(transactionUsers.groupId, session.group.groupid), isNull(transactionUsers.deleted)));
