...
function addMonthsSafely(date, monthsToAdd, originalTargetDay) {
...
export function calculateNextExecutionDate(startDate, frequency) {
...
function validateActionData(actionType, actionData, userIds, budgets) {
...
// CRUD Operations
export async function handleScheduledActionCreate(request, env) {
    return withAuth(request, env, async (session, db) => {
        ...
        const validationError = validateActionData(body.actionType, body.actionData, group.userids, group.budgets);
        if (validationError) {
        ...
        const nextExecutionDate = calculateNextExecutionDate(body.startDate, body.frequency);
...
export async function handleScheduledActionList(request, env) {
    return withAuth(request, env, async (session, db) => {
...
export async function handleScheduledActionUpdate(request, env) {
    return withAuth(request, env, async (session, db) => {
        const body = await request.json();
        if (!body.id) {
        ...
        const existingAction = await db
            .select()
            .from(scheduledActions)
            .where(and(eq(scheduledActions.id, body.id), eq(scheduledActions.userId, session.user.id)))
            .limit(1);
        if (existingAction.length === 0) {
            return new Response(JSON.stringify({ error: "Scheduled action not found" }), {
                status: 404,
                headers: { "Content-Type": "application/json" },
            });
        }
        ...
        if (body.actionData) {
            ...
            // Validate action data
            const validationError = validateActionData(existingAction[0].actionType, body.actionData, group.userids, group.budgets);
            if (validationError) {
                return new Response(JSON.stringify({ error: validationError }), {
                    status: 400,
                    headers: { "Content-Type": "application/json" },
        ...
        if (body.frequency || body.startDate) {
            ...
            updateData.nextExecutionDate = calculateNextExecutionDate(newStartDate, newFrequency);
        }
        // Update the action
        await db
...
export async function handleScheduledActionDelete(request, env) {
    return withAuth(request, env, async (session, db) => {
        const body = await request.json();
        if (!body.id) {
            return new Response(JSON.stringify({ error: "Missing action ID" }), {
...
export async function handleScheduledActionHistory(request, env) {
    return withAuth(request, env, async (session, db) => {
        const url = new URL(request.url);
        const offset = parseInt(url.searchParams.get("offset") || "0");
        const limit = Math.min(parseInt(url.searchParams.get("limit") || "10"), 50);
        ...
        if (scheduledActionId) {
            conditions.push(eq(scheduledActionHistory.scheduledActionId, scheduledActionId));
        }
        if (actionType && ["add_expense", "add_budget"].includes(actionType)) {
            conditions.push(eq(scheduledActionHistory.actionType, actionType));
        }
        if (executionStatus && ["success", "failed"].includes(executionStatus)) {
            conditions.push(eq(scheduledActionHistory.executionStatus, executionStatus));
        }
        // Get total count
        const totalCountResult = await db
            .select({ count: count() })
