...
// Handle balances
export async function handleBalances(request: Request, env: Env): Promise<Response> {
...
export async function handleBudget(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'POST') {
    return createErrorResponse('Method not allowed', 405, request, env);
  }
  try {
...
export async function handleBudgetDelete(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'POST') {
    return createErrorResponse('Method not allowed', 405, request, env);
  }
  try {
...
// Handle budget list
export async function handleBudgetList(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'POST') {
    return createErrorResponse('Method not allowed', 405, request, env);
  }
...
export async function handleBudgetMonthly(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'POST') {
    return createErrorResponse('Method not allowed', 405, request, env);
  }
  ...
  try {
    return withAuth(request, env, async (session, db) => {
      ...
      const monthlyData = await db
        .select({
          month: sql<number>`CAST(strftime('%m', added_time) AS INTEGER)`.as('month'),
...
// Handle budget total
export async function handleBudgetTotal(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'POST') {
    return createErrorResponse('Method not allowed', 405, request, env);
  }
