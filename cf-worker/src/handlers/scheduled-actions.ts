...
export async function handleScheduledActionCreate(
  ...
  return withAuth(request, env, async (session, db) => {
    ...
    // Validate input
    if (!body.actionType || !body.frequency || !body.startDate || !body.actionData) {
...
export async function handleScheduledActionUpdate(
  ...
  return withAuth(request, env, async (session, db) => {
    ...
    if (body.frequency || body.startDate) {
      ...
      updateData.nextExecutionDate = calculateNextExecutionDate(newStartDate, newFrequency as 'daily' | 'weekly' | 'monthly');
    }
...
export async function handleScheduledActionHistory(
  ...
  return withAuth(request, env, async (session, db) => {
    ...
    if (actionType && ['add_expense', 'add_budget'].includes(actionType)) {
      conditions.push(eq(scheduledActionHistory.actionType, actionType as 'add_expense' | 'add_budget'));
    }
    if (executionStatus && ['success', 'failed'].includes(executionStatus)) {
      conditions.push(eq(scheduledActionHistory.executionStatus, executionStatus as 'success' | 'failed'));
    }
