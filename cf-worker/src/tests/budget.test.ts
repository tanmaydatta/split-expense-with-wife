...
import { UserBalancesByUser, Env, CreateBudgetResponse, DeleteBudgetResponse } from '../types';
...
describe('Budget Handlers', () => {
  ...
  describe('handleBudgetCreate', () => {
    it('should create a new budget category successfully', async () => {
      // Set up test data
      await createTestUserData(env);
      await createTestSession(env);

      const request = new Request('http://example.com/.netlify/functions/budget/create', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer test-session-id',
          'Content-Type': 'application/json'
      ...
      expect(response.status).toBe(200);
      const json = await response.json() as CreateBudgetResponse;
      expect(json.message).toBe('Budget category created successfully');
      expect(json.budgetName).toBe('vacation');

      // Verify budget was added to the group
    ...
    it('should return 400 if budget name is empty', async () => {
      // Set up test data
      await createTestUserData(env);
      await createTestSession(env);
    ...
    it('should return 400 if budget already exists', async () => {
      ...
      const request = new Request('http://example.com/.netlify/functions/budget/create', {
        ...
        body: JSON.stringify({
          name: 'house', // This already exists in test data
          groupid: 1
        })
      });
      ...
      expect(json.error).toBe('Budget category already exists');
    });

    it('should return 400 for invalid budget name characters', async () => {
      // Set up test data
      await createTestUserData(env);
      await createTestSession(env);
      ...
      expect(response.status).toBe(400);
      const json = await response.json() as { error: string };
      expect(json.error).toBe('Budget name can only contain letters, numbers, hyphens, and underscores');
    });

    it('should return 401 if user is not authenticated', async () => {
      // Set up test data but don't create session
      await createTestUserData(env);
  ...
  describe('handleDeleteBudget', () => {
    it('should delete an existing budget category successfully', async () => {
      // Set up test data
      await createTestUserData(env);
      await createTestSession(env);

      const request = new Request('http://example.com/.netlify/functions/budget/delete', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer test-session-id',
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          name: 'house', // This exists in test data
          groupid: 1
        })
      });
      ...
      expect(response.status).toBe(200);
      const json = await response.json() as DeleteBudgetResponse;
      expect(json.message).toBe('Budget category deleted successfully');
      expect(json.budgetName).toBe('house');

      // Verify budget was removed from the group
      const groupStmt = env.DB.prepare('SELECT budgets FROM groups WHERE groupid = ?');
      const groupResult = await groupStmt.bind(1).first() as { budgets: string };
      const budgets = JSON.parse(groupResult.budgets);
    ...
    it('should return 400 if budget name is empty', async () => {
      ...
      expect(response.status).toBe(400);
      const json = await response.json() as { error: string };
      expect(json.error).toBe('Budget name cannot be empty');
    });
    ...
    it('should return 404 if budget does not exist', async () => {
      ...
      const json = await response.json() as { error: string };
      expect(json.error).toBe('Budget category does not exist');
    });
    ...
    it('should return 401 if user tries to delete budget from different group', async () => {
      // Set up test data
      await createTestUserData(env);
      await createTestSession(env);
