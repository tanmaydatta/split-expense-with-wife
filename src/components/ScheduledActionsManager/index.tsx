...
export const ScheduledActionsManager: React.FC = () => {
  ...
  const currencies: string[] = useMemo(() => session?.extra?.currencies || ["USD"], [session]);
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        setError(e?.errorMessage || e?.message || "Failed to create scheduled action");
        setSuccess("");
      }
    },
  });
  ...
  const paidByUserId = useStore(form.store, (s) => (s.values as any)?.actionData?.paidByUserId || "");
  ...
  React.useEffect(() => {
    ...
    const defaultShare = session?.extra?.group?.metadata?.defaultShare as Record<string, number> | undefined;
    if (defaultShare && Object.keys(defaultShare).length > 0) {
      Object.entries(defaultShare).forEach(([userId, pct]) => {
        form.setFieldValue(`actionData.splitPctShares.${userId}` as any, pct as any);
      });
    }
    ...
    <div data-test-id="scheduled-actions-manager">
      <Card className="settings-card">
        ...
        >
          ...
          {success && <SuccessContainer message={success} onClose={() => setSuccess("")} />}
          <label>Action</label>
            ...
            {(field) => (
              <Input
                ...
                onChange={(e) => {
                  ...
                  const clamped = val && val < todayAsLocalISODate ? todayAsLocalISODate : val;
              ...
              <form.Field name="actionData.budgetName">
                {(field) => (
                  <SelectBudget
                    ...
                    handleChangeBudget={(val: string) => field.handleChange(val)}
                    disabled={false}
                  />
                )}
              </form.Field>
              ...
              <form.Field name="actionData.type">
                {(field) => (
                  <CreditDebit
                    ...
                    handleChangeBudget={(val: string) => field.handleChange(val as any)}
                    disabled={false}
                  />
                )}
              </form.Field>
          ...
          <ButtonRow>
            <Button type="submit" data-test-id="sa-submit" disabled={!canSubmit || isSubmitting || createAction.isPending || (actionType === "add_expense" && !paidByUserId)}>
              {createAction.isPending || isSubmitting ? "Creating..." : "Create"}
            </Button>
          </ButtonRow>
        </FormContainer>
      </Card>
