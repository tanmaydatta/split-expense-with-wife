...
class TransactionBalanceTestHelper {
  ...
  private async getVisibleDetailsContainer(transactionId: string) {
    ...
    const hasEmptyState = await this.isEmptyBalancesState();
		...
		const amountItems = this.authenticatedPage.page.locator(
...
test.describe("Transactions and Balances", () => {
	...
	test("should handle transaction details expansion", async ({
		...
		const expense = await helper.createTestExpense(
			...
			150,
			"USD",
			{ [userId1]: newUser1Percentage, [userId2]: 100 - newUser1Percentage },
		);
	...
	test("should display balances page with real data", async ({
		...
		await helper.createTestExpense("Balance test 1", 50, "USD", {
			[userId1]: 70,
			[userId2]: 30,
		});
		await helper.createTestExpense("Balance test 2", 80, "EUR", {
			[userId1]: 40,
			[userId2]: 60,
		});
	...
	test("should handle pagination on transactions page", async ({
		...
		const showMoreButton = authenticatedPage.page.locator(
		...
		try {
			...
			await helper.verifyTransactionInList(
				...
				expenses[expenses.length - 1].currency,
			);
		} catch (_e) {
	...
	test("should display loading states during data fetching", async ({
		...
		await expect(
			authenticatedPage.page.locator('[data-test-id="expenses-container"]'),
	...
	test("should update balances correctly after expense deletion", async ({
		...
		expectedAfterAddition["Jane"]["USD"] =
			(expectedAfterAddition["Jane"]["USD"] || 0) + 40; // Jane owes John $40
		expectedAfterAddition["Jane"]["EUR"] =
			(expectedAfterAddition["Jane"]["EUR"] || 0) + 40; // Jane owes John €40
		...
		const expenseHelper = new (
			await import("../utils/expense-test-helper")
		).ExpenseTestHelper(authenticatedPage);
		...
		expectedAfterFirstDeletion["Jane"]["EUR"] =
			(expectedAfterFirstDeletion["Jane"]["EUR"] || 0) + 40; // Only €40 from second expense remains
	...
	test("should handle transactions with more than 2 people in group", async ({
		authenticatedMultiPersonPage,
	}) => {
		const helper = new TransactionBalanceTestHelper(
			authenticatedMultiPersonPage,
		);

		// Get dynamic user IDs for the multi-person group
		const userPercentages = await getCurrentUserPercentages(
			authenticatedMultiPersonPage,
		);
		const userIds = Object.keys(userPercentages);
