...
export async function getCurrentUserId(
	...
	const welcomeElement = testHelper.page.locator(
		'[data-test-id^="sidebar-welcome-"]',
	);
	...
	throw new Error(
		"Could not get current user ID from sidebar welcome message. Make sure user is logged in and the page has loaded completely.",
	);
}
...
export class TestHelper {
	...
	async login(user: TestUser): Promise<void> {
		...
		await this.page.waitForSelector('[data-test-id="dashboard-container"]', {
	...
	async getTransactions(): Promise<any[]> {
		...
		for (const transaction of transactions) {
			const description = await transaction
				.locator(".transaction-description")
				.textContent();
			const amount = await transaction
				.locator(".transaction-amount")
				.textContent();
	...
	async isAuthenticated(): Promise<boolean> {
		try {
			...
			await this.page.waitForSelector(".SidebarHeader", {
