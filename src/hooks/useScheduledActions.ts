...
export function useScheduledActionHistory(
  ...
  if (filters?.scheduledActionId) params.set("scheduledActionId", filters.scheduledActionId);
...
export function useScheduledActionDetails(id?: string) {
  return useQuery({
    ...
    enabled: !!id,
    queryFn: async () => {
      const resp = await apiInstance.get(`/scheduled-actions/details?id=${encodeURIComponent(id || "")}`);
      return resp.data;
    },
  });
}
...
export function useScheduledActionHistoryDetails(id?: string) {
  return useQuery({
    ...
    enabled: !!id,
    queryFn: async () => {
      const resp = await apiInstance.get(`/scheduled-actions/history/details?id=${encodeURIComponent(id || "")}`);
      return resp.data;
    },
  });
}
